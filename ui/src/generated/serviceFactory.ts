...
export interface GqlService {
  ...
  ) => Promise<ApolloQueryResult<fetchFilters>>;
  watchfetchFilters: (
    options?: Omit<QueryOptions, 'query' | 'variables'>
  ) => ObservableQuery<fetchFilters>;

  fetchProducts: (
    variables: fetchProductsVariables,
    options?: Omit<QueryOptions<fetchProductsVariables>, 'query' | 'variables'>
  ) => Promise<ApolloQueryResult<fetchProducts>>;
  watchfetchProducts: (
    variables: fetchProductsVariables,
    options?: Omit<QueryOptions<fetchProductsVariables>, 'query' | 'variables'>
  ) => ObservableQuery<fetchProducts, fetchProductsVariables>;

  likeProduct: (
    variables: likeProductVariables,
    options?: Omit<
      MutationOptions<likeProduct, likeProductVariables>,
      'mutation' | 'variables'
    >
  ) => Promise<FetchResult<likeProduct>>;
}
...
export function createService(client: ApolloClient<any>): GqlService {
  return {
    ...
    fetchProducts: (
      variables: fetchProductsVariables,
      options: Omit<
        QueryOptions<fetchProductsVariables>,
        'query' | 'variables'
      > = {}
    ) => {
...
export const fetchFiltersQuery = gql`
  query fetchFilters {
    catalog {
      __typename
      filters {
...
export const fetchProductsQuery = gql`
  query fetchProducts($brandIds: [String!], $pageInfo: PaginationInput) {
    catalog {
      __typename
      products(
        input: { pageInfo: $pageInfo, filters: { brandIds: $brandIds } }
      ) {
        Price {
          __typename
          amount
...
export const likeProductMutation = gql`
  mutation likeProduct($liked: Boolean!, $productId: String!) {
    catalog {
      __typename
      likeProduct(input: { productId: $productId, liked: $liked }) {
        __typename
        success
      }
    }
